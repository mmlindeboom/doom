...
(defvar linear-to-org--query
  "{\"query\": \"query { issues(first: 100) { nodes { id identifier title description url state { name } priority team { name } project { name } labels { nodes { name } } createdAt updatedAt dueDate assignee { name email } } } }\"}")
...
(defun linear-to-org--make-request ()
  ...
  (unless (string-empty-p linear-to-org-api-key)
    ...
    (request
     ...
     :data (json-encode `(("query" . "query { issues(first: 100) { nodes { id identifier title description url state { name } priority team { name } project { name } labels { nodes { name } } createdAt updatedAt dueDate assignee { name email } } } }")))
     :parser 'json-read
...
(defun linear-to-org--process-issues (issues)
  ...
  (with-current-buffer (find-file-noselect linear-to-org-target-file)
    ...
    ;; Save the buffer
    (save-buffer)
    (message "Linear issues imported to %s" linear-to-org-target-file)))
...
(defun linear-to-org-debug-request ()
  ...
  (unless (string-empty-p linear-to-org-api-key)
    ...
    (request
     ...
     :headers `(("Content-Type" . "application/json")
                ("Authorization" . ,linear-to-org-api-key))
     :data (json-encode `(("query" . "query { viewer { id name } }")))
     :parser 'json-read
...
;; Add keybinding for Linear to Org
(map! :leader

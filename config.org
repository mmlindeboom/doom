...
(defcustom linear-to-org-email "mattlindeboom@unspun.io"
  "Email to filter Linear issues by assignee."
  :type 'string
  :group 'linear-to-org)
...
(defun linear-to-org--make-request ()
  ...
  (unless (string-empty-p linear-to-org-api-key)
    ...
    (let ((query (format "query { issues(first: 100, filter: {assignee: {email: {eq: \"%s\"}}}) { nodes { id identifier title description url state { name } priority team { name } project { name } labels { nodes { name } } createdAt updatedAt dueDate assignee { name email } } } }" linear-to-org-email)))
...
DATA contains the response data."
  (let* ((response-data (request-response-data (plist-get data :response))))
    ...
    (if (and response-data
        ...
        (let ((issues (alist-get 'nodes (alist-get 'issues (alist-get 'data response-data)))))
          (if (and issues (vectorp issues) (> (length issues) 0))
              (linear-to-org--process-issues issues)
            (message "No issues found in Linear for email %s" linear-to-org-email)))
...
(defun linear-to-org--process-issues (issues)
  ...
  (message "Processing %d issues from Linear" (length issues))
  ...
  (let ((target-file (if (file-name-absolute-p linear-to-org-target-file)
    ...
    (dolist (issue issues)
      (let* ((id (alist-get 'identifier issue))
             ...
             (state (when (alist-get 'state issue)
                      (alist-get 'name (alist-get 'state issue))))
             (org-state (linear-to-org--map-state (or state "Todo")))
             (priority (alist-get 'priority issue))
             (org-priority (linear-to-org--map-priority priority))
             (description (alist-get 'description issue))
             (url (alist-get 'url issue))
             (team (when (alist-get 'team issue)
                     (alist-get 'name (alist-get 'team issue))))
             (project (when (alist-get 'project issue)
                        (alist-get 'name (alist-get 'project issue))))
             ...
             (assignee-name (when (alist-get 'assignee issue)
                              (alist-get 'name (alist-get 'assignee issue))))
             (assignee-email (when (alist-get 'assignee issue)
                               (alist-get 'email (alist-get 'assignee issue)))))

        ;; Try to safely extract labels
        (when-let* ((label-data (alist-get 'labels issue))
                    (nodes (alist-get 'nodes label-data)))
          (unless (eq nodes [])
            (when (vectorp nodes)
              (setq labels
                    (seq-filter #'stringp
...
(defun linear-to-org-set-email (email)
  "Set the Linear EMAIL to filter issues by assignee."
  (interactive "sEnter Linear email: ")
  (setq linear-to-org-email email)
  (message "Linear email set to %s" email))
...
;; (map! :leader
;;       (:prefix ("ol" . "Linear")
;;        :desc "Fetch Linear issues" "f" #'linear-to-org-fetch-issues
;;        :desc "Set Linear email" "e" #'linear-to-org-set-email
;;        :desc "Set Linear API key" "k" #'linear-to-org-set-api-key
;;        :desc "Debug Linear API" "d" #'linear-to-org-debug-request))
#+end_src

#+RESULTS:
: linear-to-org-debug-request

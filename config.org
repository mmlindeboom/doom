...
(defvar linear-to-org--query
  "{\"query\": \"query { issues(first: 10) { nodes { id identifier title description url state { name } priority team { name } project { name } labels { nodes { name } } createdAt updatedAt dueDate assignee { name email } } } }\"}")
